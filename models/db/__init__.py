...
class Finder:
    ...
    def dbConnection():
        if not Finder._db:
            Finder._db = torndb.Connection(
                host=configs.mysql_slave, database=configs.mysql_dbname,
                user=configs.mysql_user, password=configs.mysql_password)
        return Finder._db
...
class base:
    ...
    def dbConnection():
        if not base._db:
            base._db = torndb.Connection(
                host=configs.mysql_master, database=configs.mysql_dbname,
                user=configs.mysql_user, password=configs.mysql_password)
        return base._db
    ...
    def update(self, valuedict, where, value):
        valuecode = []
        for i in valuedict.keys():
            if i == 'id':
                pass
            data = '"' + str(valuedict[i]) + '"'
            string = " `"+i+"`="+data
            valuecode.append(string)
        joinstr = ",".join(valuecode)
        # return str(type(value))
        sql = "UPDATE `"+self.db_name+"` SET "+joinstr+" WHERE `"+where+"`="+"'"+value+"'"
        return self.db.execute(sql)
        # return self.db_name

    def timeline(self, user):
        # sql = "SELECT * from `report` where `creator`='"+user+"' union SELECT * from `response`  where `towho`='"+user+"' order by `createtime`"
        # sql = "select `content`,`createtime`,`creator` from `report` where `creator`='"+user+"' union select `content`,`createtime`,`groupid` from `response` where `towho`='"+user+"' order by `createtime`"
        sql = 'select [content],[createtime],[creator] from [report] where [creator]=%s union select [content],[createtime],[groupid] from [response] where [towho]=%s order by [createtime]', (user, user)
        return self.db.execute(sql)
    ...
    def add(self):
        ...
        # `session_key`,`session_value`,`session_left_time`,`session_time`)
        # VALUES ( %s,%s,%s,%s)
        parameters = []
        key = []
        str = []
    ...
    def add_many(self):
        ...
        for k in self.attr.keys():
            if not isinstance(self.attr[k], list):
                return False
            length = len(self.attr[k])
            key.append('`' + k + '`')

        for i in range(length):
            for j in self.attr.keys():
                if len(self.attr[j]) != length:
                    return False
                value.append("'"+self.attr[j][i]+"'")
            key_value.append('('+','.join(value)+')')
            value = []

  
        print sql + '(' + ','.join(key) + ') VALUES ' + ','.join(key_value)
